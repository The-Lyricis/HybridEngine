...
- RenderSystem ????? `renderFrame` ?????????? `buildRenderPacket(...) + executePasses(...) + executeForwardPass(...)`?CPU ????? GPU ?????????
- Mesh / Material ???????RenderSystem ??? MeshGPUCache / MaterialGPUCache????? `AssetID` ?????
- ?? cube ????? Mesh ????????????? cube VAO ????????????
- shader ??????????albedo / metallic / roughness / ao / emissive + normal map?`MeshVertex::tangent` ??? `vec4` ???
- ????? 1 ???? + ????????????? `TransformComponent` ????????????? `direction` ???

???????????

- `renderFrame(...)` ????? init / resize / build packet / execute??????????????????
- MR ?????????? `(1,1)`?shader ????????????? roughness ????? 1?
- emissive ???????? emissive color????? albedo?
- normal map ???? tangent ??????? TBN ????? NaN / ???
- fallback cube ??????? primitive=cube ???????????? DrawItem ?? cube ???

## 2026-03-10 ??? Play/Edit ????

- Scene Picking ? ID attachment ?????? `uint` ????? `GL_R32UI` ?????
- Picking ??? `entt entity 0` ???? 0???????
  - GPU ???`encoded = raw + 1`
  - ?????`0`
  - CPU ???`0 -> invalid`??? `id -> id - 1`
- ??????? `kInvalidEntityID` ??????????? `0` ??????????????? 0??
- gizmo / helper pass ???? `COLOR0`????? EntityID attachment?
- ???????? selection / pending pick / selected_entity_id?????? ID ???????
- `Play` ? Scene ???? gizmo / inspector / picking ????? runtime scene?????? editor scene?
- `Play -> Stop` ??????? UI selection / pick request ??? context??? runtime scene ????? UI ?????
- ?? set scene / shutdown ??? `Play` ?????? `exitPlayMode()`??? runtime scene ??????????

## ??????????

1. ? Frame / Light uniform ???? UBO?????????????????????
2. ???? pass ???? prepare / sort / execute ????????
3. ????? shadow pass?selection outline?post process??? RenderPipeline / RenderGraph ???
