...
namespace Hybrid
{
    ...
    void HybridEngine::initialize()
    {
        ...
        // 1) 确保 Debug.scene 存在
        {
            const auto& pctx = Hybrid::ProjectService::Get();
            const std::filesystem::path debugScenePath = pctx.assets / "Scenes" / "DemoA.scene";

            // 2) 读取 Debug.scene（Transform/层级）
            if (!SceneSerializer::DeserializeFromFile(*scene, debugScenePath))
            {
                HBD_CORE_WARN("Failed to load Debug.scene, fallback to empty scene: {}", debugScenePath.string());
            }
            else
            {
                HBD_CORE_INFO("Loaded debug scene from file: {}", debugScenePath.string());
            }
        }
